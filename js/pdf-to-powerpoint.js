...
class PDFToPowerPointConverter {
    ...
    if (this.convertedPptxBlob) {
        ...
        <div class="file-download-container">
          <div class="file-info-row">
            ...
            <div class="file-details">
              <span class="file-name">converted.pptx</span>
      ...
      uploadContent.innerHTML = this.selectedFiles.length > 0 ? `
        ...
        <div class="advanced-options">
          ...
          <div class="option-group">
            <label for="elements">Convert Elements</label>
            <select id="elements" class="advanced-option">
        ...
        <div class="action-buttons">
          <button class="convert-button" onclick="window.pdfToPptConverter.convertToPowerPoint()">
  ...
  async convertToPowerPoint() {
    ...
    try {
      ...
      const layout = document.getElementById('layout')?.value || 'auto';
      const quality = document.getElementById('quality')?.value || 'medium';
      const elements = document.getElementById('elements')?.value || 'all';
      ...
      params.add('DocumentLayout', layout.toUpperCase());
      params.add('ConvertElements', elements.toUpperCase());
      
      const result = await convertApi.convert('pdf', 'pptx', params);
      const fileUrl = result.files[0].Url;

      // Download the converted PowerPoint
      const response = await fetch(fileUrl);
      this.convertedPptxBlob = await response.blob();
      this.updateUploadBoxContent();
    } catch (error) {
      ...
      convertButton.innerHTML = '<i class="fas fa-exchange-alt"></i> Convert to PowerPoint';
    }
  }

  downloadPowerPoint() {
    if (this.convertedPptxBlob) {
      const fileName = this.selectedFiles[0]?.name.replace(/\.[^/.]+$/, '') + '.pptx';
      saveAs(this.convertedPptxBlob, fileName || 'converted.pptx');
    }
  }
}

document.addEventListener('DOMContentLoaded', () => {
  window.pdfToPptConverter = new PDFToPowerPointConverter();
});