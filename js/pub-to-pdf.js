...
class PubToPDFConverter {
  ...
  setupEventListeners() {
    ...
    this.uploadBox.addEventListener('drop', (e) => {
      e.preventDefault();
      this.uploadBox.style.borderColor = '#e2e8f0';
      const files = Array.from(e.dataTransfer.files).filter(file => file.name.toLowerCase().endsWith('.pub'));
      if (files.length > 0) {
    ...
    this.fileInput.addEventListener('change', (e) => {
      const files = Array.from(e.target.files).filter(file => file.name.toLowerCase().endsWith('.pub'));
      if (files.length > 0) {
        this.handleFileSelection(files);
      }
    });
  ...
  updateUploadBoxContent() {
    ...
    if (this.convertedPdfBlob) {
      ...
      uploadContent.innerHTML = this.selectedFiles.length > 0 ? `
        ...
        <div class="advanced-options">
          ...
          <div class="option-group">
            ...
            <select id="colorMode" class="advanced-option">
        ...
        <div class="action-buttons">
          <button class="convert-button" onclick="window.pubToPdfConverter.convertToPDF()">
      ...
      ` : `
        ...
        <h3>Drop your Publisher file here</h3>
        <p>or</p>
  ...
  async convertToPDF() {
    ...
    try {
      ...
      const pageSize = document.getElementById('pageSize')?.value || 'original';
      ...
      params.add('ColorMode', colorMode.toUpperCase());
      params.add('PageSize', pageSize.toUpperCase());
      
      const result = await convertApi.convert('pub', 'pdf', params);
  ...
  downloadConvertedPDF() {
    if (this.convertedPdfBlob) {
      const fileName = this.selectedFiles[0]?.name.replace(/\.pub$/i, '.pdf');
...
document.addEventListener('DOMContentLoaded', () => {
  window.pubToPdfConverter = new PubToPDFConverter();
});